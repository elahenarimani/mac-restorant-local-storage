...
function addSumCost() {//مجموع سفارشات
    ...
    syncDataToLocalStorage()
    const totalOrderFromLocalStorage = localStorage.getItem("foodData");
    const extractedtotalOrder = JSON.parse(totalOrderFromLocalStorage);
    totalOrderPay.innerHTML = extractedtotalOrder + ` تومان`;
    console.log(extractedtotalOrder);
    if (flag == true) {
        payService(extractedtotalOrder);
        console.log(extractedtotalOrder)
    }
    return extractedtotalOrder;   
}
function payService(paySum) {//حق سرویس
    ...
    service = (paySum * 0.15);
    syncDataToLocalStorage()
    const serviceFromLocalStorage = localStorage.getItem("foodData");
    const extractedFromLocalStorage = JSON.parse(serviceFromLocalStorage)
...
function getDiscount() {
    ...
    let discountAmount;
    syncDataToLocalStorage();
    const discountFromLocalStorage = localStorage.getItem("foodData");
    const extractedDiscount = JSON.parse(discountFromLocalStorage)
    if (discountCode.value == 'elahe1402') {
        extractedDiscount = (completeOrder * 0.5);
        result = (completeOrder - extractedDiscount) + tax;
        const resultFromLocalStorage = localStorage.getItem("foodData");
        const extractedResult = JSON.parse(resultFromLocalStorage)
        costPayAble.innerHTML = extractedResult + `تومان`;
        discountItem.innerHTML = extractedDiscount + ` تومان`;
    } else {
